          ...
          <div className="space-y-2">
            {sortedInterventions.map((iv) => (
              <InterventionRow
                key={iv.id}
                intervention={iv}
                expanded={isInterventionExpanded(iv.id)}
                onToggle={() => toggleIntervention(iv.id)}
...
function InterventionRow({
  ...
  const journalCount = iv._count?.journalEntries ?? 0;

  // Lazy-fetch journal entries only when this row is expanded.
  const { data: journalEntries = [] } = useJournalEntries(iv.id, {
    enabled: expanded,
  });
      ...
      {expanded && (
        ...
        >
          ...
          {journalEntries.length > 0 && (
            <div className="space-y-1.5">
              <span className="text-xs font-mono text-muted-foreground uppercase">
                Journal de bord ({journalEntries.length})
              </span>
              <JournalTimeline updates={journalEntries} />
            </div>
          )}
              ...
              <div className="flex flex-wrap gap-1.5">
                {photos.map((url, i) => (
                  <button
                    key={i}
                    type="button"
                    onClick={() => onOpenLightbox(photos.map(getImageUrl), i)}
