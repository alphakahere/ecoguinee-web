...
export const campaignsService = {
	...
	): Promise<ApiPaginatedResponse<ApiCampaign>> {
		const { data } = await api.get<
			ApiPaginatedResponse<ApiCampaign> | ApiCampaign[]
		>("/campaigns", { params: filters });
	...
	async getBySlug(slug: string): Promise<ApiCampaign> {
		const { data } = await api.get<ApiCampaign>(
			`/campaigns/slug/${slug}`,
		);
		return data;
	},
	...
	async update(
		...
		const { data } = await api.patch<ApiCampaign>(
			`/campaigns/${id}`,
			payload,
		);
		return data;
