...
export default function AdminUsersPage() {
  ...
  const filters = useMemo(
    ...
    [debouncedSearch, roleTab, statusFilter, page],
  );
  ...
  const isSubmitting = createUser.isPending || updateUser.isPending || updateUserStatus.isPending || deleteUser.isPending;
  ...
  const columns: Column<User>[] = [
    {
      ...
      render: (u) => (
        <div className="flex items-center gap-3">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-primary/10 text-xs font-semibold text-primary">{initials(u.name)}</span>
          <div>
    ...
    {
      key: 'role', label: 'Rôle',
      render: (u) => { const m = ROLE_META[u.role]; return <span className={`inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-xs font-medium ${m.bg} ${m.color}`}><span className="h-1.5 w-1.5 shrink-0 rounded-full" style={{ background: m.dot }} />{m.label}</span>; },
    },
    {
      key: 'status', label: 'Statut',
      render: (u) => <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-medium ${STATUS_COLORS[u.status]}`}>{STATUS_LABELS[u.status]}</span>,
    },
    { key: 'createdAt', label: 'Inscription', render: (u) => <span className="font-mono text-xs text-muted-foreground">{formatDateShort(u.createdAt)}</span> },
    { key: 'lastLogin', label: 'Dernière conn.', render: (u) => <span className="font-mono text-xs text-muted-foreground">{u.lastLogin ? formatDateShort(u.lastLogin) : '—'}</span> },
    {
      ...
      render: (u) => (
        <div className="flex items-center justify-end gap-1">
          <button type="button" title="Modifier" onClick={() => openEdit(u)} className="rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-foreground"><Pencil className="h-4 w-4" /></button>
          <button type="button" title={u.status === 'ACTIVE' ? 'Désactiver' : 'Activer'} disabled={u.id === currentUser?.id} onClick={() => handleToggleActive(u)} className="rounded-lg p-2 text-muted-foreground hover:bg-muted hover:text-foreground disabled:opacity-40">
            {u.status === 'ACTIVE' ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
          </button>
          <button type="button" title="Supprimer" disabled={u.id === currentUser?.id} onClick={() => handleDeleteRow(u)} className="rounded-lg p-2 text-muted-foreground hover:bg-destructive/10 hover:text-destructive disabled:opacity-40"><Trash2 className="h-4 w-4" /></button>
        </div>
      ),
    },
  ];
    ...
    <div className="flex flex-col gap-4">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-muted-foreground">{total} utilisateur{total !== 1 ? 's' : ''}</p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button type="button" variant="outline" onClick={onExportCsv} className="font-mono text-xs"><Download className="mr-2 h-4 w-4" />Exporter CSV</Button>
          <Button type="button" onClick={openCreate} className="font-mono text-xs"><Plus className="mr-2 h-4 w-4" />Nouvel Utilisateur</Button>
        </div>
      </div>
      <div className="flex flex-col gap-3 lg:flex-row lg:flex-wrap lg:items-center lg:gap-3">
        <SearchInput value={search} onChange={(v) => { setSearch(v); setPage(1); }} placeholder="Nom, email, territoire…" className="w-full max-w-md" />
        <div className="flex flex-wrap gap-2">
          {roleTabs.map((t) => (
            <button key={t.id} type="button" onClick={() => { setRoleTab(t.id); setPage(1); }}
              className={cn('rounded-full px-3 py-1.5 text-xs font-mono transition-colors', roleTab === t.id ? 'bg-primary text-primary-foreground hover:bg-primary/90' : 'border border-border bg-background text-muted-foreground hover:bg-muted/60')}
            >{t.label}</button>
          ))}
        </div>
        <div className="lg:ml-auto">
          <Select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value); setPage(1); }} className="min-w-[180px]">
